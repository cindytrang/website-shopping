...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const productList = document.getElementById("chooseProducts")


  console.log("products are :" + productList.value); 
  ...
  function filterItems() {
    console.log("items are being filtered")
    ...
    const filteredProducts = imagesArr.filter(item => {
      return item[0] && (category === "" || item[1] === category);
    });
    
    displayItemsSearch(filteredItems);

    displayFilteredProducts(filteredProducts)
  }
  
  // Function to display filtered items
  function displayItemsSearch(items) {
  ...
  function displayFilteredProducts(items) {
    const products = document.getElementById("chooseProducts")
    products.setAttribute(items)
    initProducts(items);
 ...
 // searchButton.addEventListener("click", filterItems);
  ...
  categoryRadios.forEach(radio => {s
